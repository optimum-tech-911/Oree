          ...
          <>
            ...
            >
              ...
              <div className="assistant-scroll scrollbar-thin flex-1 space-y-4 overflow-y-auto p-4 sm:p-5">
                {messages.map((message) => (
                  <div key={message.id} className={cn("flex", message.role === "user" ? "justify-end" : "justify-start")}>
                    <div className={cn("max-w-[90%] rounded-[22px] px-4 py-3 text-sm leading-6", message.role === "user" ? "rounded-br-md bg-[var(--blue)] text-white shadow-[0_12px_30px_rgba(36,87,255,.18)]" : "rounded-bl-md border border-[var(--line)] bg-white text-[color:var(--ink)] shadow-[0_10px_35px_rgba(11,18,32,.05)]")}>
                      <p>{message.text}</p>
                      {message.actions?.length ? <div className="mt-3 grid gap-2">{message.actions.map((action) => <button key={`${message.id}-${action.href}`} data-assistant-response-action type="button" onClick={() => { navigate(action.href); closeAssistant(); }} className="group flex items-center justify-between gap-3 rounded-[15px] bg-[var(--ink)] px-3.5 py-3 text-left text-xs font-extrabold transition hover:-translate-y-0.5 hover:bg-[var(--blue)]"><span className="foreground-on-dark relative z-10">{action.label}</span><ChevronRight className="foreground-on-dark size-3.5 transition group-hover:translate-x-1" /></button>)}</div> : null}
                      {message.role === "assistant" ? <button type="button" onClick={() => { openDirectContact(); closeAssistant(); }} className="mt-3 flex w-full items-center justify-between gap-3 rounded-[15px] border border-[var(--mint)]/35 bg-[var(--mint-soft)] px-3.5 py-3 text-left text-xs font-extrabold text-[color:var(--ink)] transition hover:-translate-y-0.5 hover:border-[var(--mint)]"><span className="flex items-center gap-2"><PhoneCall className="size-3.5" />Contacter l’équipe directement</span><ChevronRight className="size-3.5" /></button> : null}
                ...
                {messages.length <= 1 ? <div className="space-y-4 pt-2"><button type="button" onClick={() => { openDirectContact(); closeAssistant(); }} className="group flex w-full items-center justify-between rounded-[20px] bg-[var(--blue)] p-4 text-left shadow-[0_16px_38px_rgba(36,87,255,.18)] transition hover:-translate-y-0.5"><span className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-[13px] bg-white text-[color:var(--ink)]"><PhoneCall className="size-4" /></span><span><span className="foreground-on-dark block text-sm font-extrabold">Parler directement à l’équipe</span><span className="foreground-on-dark-muted mt-1 block text-[10px]">Appel, SMS, e-mail, WhatsApp ou WhatsApp Business.</span></span></span><ChevronRight className="foreground-on-dark size-4 transition group-hover:translate-x-1" /></button><div className="grid gap-2">{quickPrompts.map((prompt) => <button key={prompt} onClick={() => ask(prompt)} className="group flex items-center justify-between rounded-[18px] border border-[var(--line)] bg-white px-4 py-3.5 text-left text-xs font-extrabold text-[color:var(--ink)] shadow-[0_8px_28px_rgba(11,18,32,.035)] transition hover:-translate-y-0.5 hover:border-[var(--blue)]/28"><span>{prompt}</span><ArrowRight className="size-3.5 text-[color:var(--muted)] transition group-hover:translate-x-1 group-hover:text-[color:var(--blue)]" /></button>)}</div><div className="rounded-[22px] border border-[var(--line)] bg-[var(--ink)] p-4"><p className="foreground-on-dark-muted text-[9px] font-extrabold uppercase tracking-[.14em]">Ce que je peux faire</p><div className="mt-3 grid grid-cols-3 gap-2">{[{ icon: Compass, label: "Naviguer" }, { icon: FileSearch, label: "Expliquer" }, { icon: ListChecks, label: "Agir" }].map((capability) => { const Icon = capability.icon; return <div key={capability.label} className="rounded-[15px] border border-white/8 bg-white/[.045] p-3"><Icon className="size-4 text-[color:var(--mint)]" /><p className="foreground-on-dark mt-3 text-[10px] font-extrabold">{capability.label}</p></div>; })}</div><p className="foreground-on-dark-muted mt-3 text-[9px] leading-4">Je retrouve une page, explique un élément du parcours et propose l'action suivante sans perdre votre contexte.</p></div></div> : null}
                <div ref={bottomRef} />
              </div>
