...
export function AppLayout() {
  ...
  const sidebar = (
    <div className="flex h-full flex-col text-white">
      ...
      <div className="mx-4 mb-5 rounded-[20px] border border-white/8 bg-white/[.045] p-4"><div className="flex items-center justify-between"><div><p className="text-[9px] font-extrabold uppercase tracking-[.14em] text-white/72">Projet actif</p><p className="mt-1.5 text-sm font-extrabold">Studio Horizon</p></div><span className="rounded-full bg-[var(--mint)] px-2.5 py-1 text-[9px] font-extrabold text-[color:var(--ink)]">64%</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/7"><div className="h-full w-[64%] rounded-full bg-[linear-gradient(90deg,var(--mint),var(--sky))]" /></div></div>
        ...
        {clientNavigation.map((item) => {
          ...
          return (
            <NavLink end={item.href === "/app"} key={item.href} to={item.href} onClick={() => setMobileOpen(false)} className={({ isActive }) => cn("group flex items-center gap-3 rounded-[15px] px-3 py-3 text-[13px] font-bold transition", isActive ? "bg-white text-[color:var(--ink)] shadow-[0_12px_34px_rgba(11,18,32,.16)]" : "text-white/72 hover:bg-white/7 hover:text-white") }>
              ...
              {item.href === "/app/messages" ? <span className="ml-auto grid size-5 place-items-center rounded-full bg-[var(--coral)] text-[9px] text-white">1</span> : null}
            </NavLink>
          );
        })}
      </nav>
      <div className="m-3 rounded-[20px] border border-white/8 bg-white/[.045] p-3"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-full bg-[var(--mint)] text-[11px] font-extrabold text-[color:var(--ink)]">PP</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-extrabold">{user ? `${user.firstName} ${user.lastName}` : "Porteur de projet"}</p><p className="mt-0.5 truncate text-[9px] text-white/72">{user?.demo !== false ? "Mode démonstration" : user.email}</p></div><button type="button" onClick={() => void signOut()} className="grid size-9 place-items-center rounded-full text-white/72 transition hover:bg-white/8 hover:text-white" title="Se déconnecter"><LogOut className="size-4" /></button></div></div>
    </div>
  );
