...
import { ArrowRight, ArrowUpRight, ListChecks, ShieldCheck } from "lucide-react";
  ...
  return (
    <footer className="relative z-[2] overflow-hidden bg-[var(--night)] pb-24 pt-8 text-white lg:pb-8">
      ...
      <div className="container-shell relative">
        ...
        <div className="flex flex-col gap-4 pt-7 text-xs text-white/72 sm:flex-row sm:items-center sm:justify-between"><div className="space-y-1.5"><p>© 2026 Orée.</p><p>Orée Entreprises est un service exploité par Optimum Tech — SIRET : 98898880400015</p></div><div className="flex flex-wrap gap-5"><Link className="transition hover:text-white" to="/confidentialite">Confidentialité</Link><Link className="transition hover:text-white" to="/mentions-legales">Mentions légales</Link><Link className="group inline-flex items-center gap-1 transition hover:text-white" to="/diagnostic">Commencer <ArrowRight className="size-3 transition group-hover:translate-x-1" /></Link></div></div>
      </div>
    </footer>
  );
}
