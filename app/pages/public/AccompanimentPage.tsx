import { CalendarCheck2, FileSearch, Handshake, MessageCircleMore, ShieldCheck } from "lucide-react";
    ...
    <>
      <section className="hero-grid surface-noise relative overflow-hidden bg-[var(--night)] pb-20 pt-34 text-white sm:pt-42 lg:min-h-[820px] lg:pb-24 lg:pt-44">
        ...
        <div className="container-shell relative z-10 grid items-center gap-14 lg:grid-cols-[1fr_.82fr] lg:gap-20">
          <div><Badge className="border-white/12 bg-white/[.06] text-white/68"><Handshake className="size-3.5 text-[color:var(--mint)]" />Accompagnement humain · {commercialOffers.companyCreation.priceLabel}</Badge><h1 className="mt-7 max-w-5xl text-balance text-[clamp(3.5rem,7vw,8.2rem)] font-semibold leading-[.89] tracking-[-.077em]">Un accompagnement humain <span className="gradient-text">inclus dans la création.</span></h1><p className="mt-7 max-w-2xl text-pretty text-lg leading-8 text-white/72 sm:text-xl">La plateforme organise les informations du projet afin que les échanges portent sur les décisions, les incohérences et les points qui nécessitent une validation. Greffe, annonce légale et corrections du dossier sont compris.</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><ButtonLink to="/diagnostic" variant="dark" size="lg" arrow>{commercialOffers.companyCreation.ctaLabel}</ButtonLink><ButtonLink to="/rendez-vous" variant="ghost" size="lg" className="border border-white/12 text-white hover:bg-white/[.08]">Réserver un échange</ButtonLink></div></div>
      ...
      <Section className="bg-white/55 pt-14 sm:pt-18">
        <div className="container-shell"><CompanyOfferCard trackingLocation="accompaniment_offer" /></div>
      </Section>
