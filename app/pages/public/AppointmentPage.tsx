...
const days = Array.from({ length: 12 }, (_, index) => {
  ...
  return date;
}).filter((date) => ![0, 6].includes(date.getDay())).slice(0, 5).map((date) => ({
  value: date.toISOString().slice(0, 10),
  date: new Intl.DateTimeFormat("fr-FR", { day: "2-digit" }).format(date),
  day: new Intl.DateTimeFormat("fr-FR", { weekday: "short" }).format(date),
  label: new Intl.DateTimeFormat("fr-FR", { weekday: "long", day: "numeric", month: "long", year: "numeric" }).format(date),
}));
...
export default function AppointmentPage() {
  ...
  const summary = useMemo(() => `${selectedDay?.label ?? "Date à confirmer"} à ${slot}`, [selectedDay?.label, slot]);
  const calendarHeading = useMemo(() => selectedDay ? new Intl.DateTimeFormat("fr-FR", { month: "long", year: "numeric" }).format(new Date(`${selectedDay.value}T12:00:00`)) : "Dates proposées", [selectedDay]);
    ...
    <section className="hero-grid surface-noise relative min-h-screen overflow-hidden bg-[var(--night)] pb-20 pt-34 text-white sm:pt-42 lg:pb-28 lg:pt-44">
      ...
      <div className="container-shell relative z-10 grid gap-12 lg:grid-cols-[.72fr_1.28fr] lg:items-start lg:gap-18">
        <div className="lg:sticky lg:top-32">
          <Badge className="border-white/12 bg-white/[.06] text-white/70"><CalendarDays className="size-3.5 text-[color:var(--mint)]" />Démonstration de rendez-vous</Badge>
          ...
          <div className="mt-10 grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            {[[MessageSquareText, "Contexte préparé", "La discussion peut reprendre les éléments du diagnostic avec votre accord."], [Clock3, "Durée à confirmer", "Le format réel dépendra du calendrier et du service connectés."], [ShieldCheck, "Aucune fausse réservation", "Cette interface n'envoie pas d'événement à un calendrier externe."]].map(([Icon, title, description]) => { const Component = Icon as typeof Check; return <div key={String(title)} className="flex gap-4 rounded-[22px] border border-white/[.08] bg-white/[.035] p-4 backdrop-blur"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-white/[.08]"><Component className="size-5 text-[color:var(--mint)]" /></span><div><p className="font-semibold text-white/88">{String(title)}</p><p className="mt-1 text-xs leading-5 text-white/72">{String(description)}</p></div></div>; })}
          </div>
            ...
            {booked ? (
              ...
              <motion.div key="picker" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid lg:grid-cols-[1fr_320px]">
                ...
                <aside className="border-t border-white/[.08] bg-black/18 p-5 lg:border-l lg:border-t-0 sm:p-7">
                  ...
                  <Button variant="dark" className="mt-6 w-full" size="lg" onClick={() => { setBooked(true); analytics.track("demo_appointment_selected", { day, slot, type }); }}>Simuler la confirmation</Button><p className="mt-3 text-center text-[10px] leading-4 text-white/72">Démonstration sans connexion calendrier. Aucune conversion de réservation n'est comptabilisée.</p>
                </aside>
              </motion.div>
            )}
          </AnimatePresence>
