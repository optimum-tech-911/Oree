...
import { ArrowLeft, ArrowRight, BriefcaseBusiness, Building2, CalendarClock, Check, CircleAlert, Clock3, GitBranch, ListChecks, LoaderCircle, LockKeyhole, RotateCcw, Search, UsersRound } from "lucide-react";
    ...
    <aside className="hidden xl:block">
      <div className="sticky top-26 space-y-4">
        <div className="route-map overflow-hidden rounded-[28px] border border-[var(--line)] bg-white p-5 shadow-soft">
          <div className="flex items-center justify-between"><div><p className="text-[9px] font-extrabold uppercase tracking-[.14em] text-[color:var(--muted)]">Projet en direct</p><p className="mt-2 text-lg font-extrabold tracking-[-.04em]">Votre feuille de route</p></div><span className="grid size-10 place-items-center rounded-[12px] border border-[var(--line)] bg-white"><ListChecks className="size-4" /></span></div>
        ...
        <button type="button" onClick={() => window.dispatchEvent(new CustomEvent("oree:assistant-open"))} className="group flex w-full items-center gap-3 rounded-[18px] border border-[var(--line)] bg-white p-4 text-left transition hover:border-[var(--blue)]/30"><Search className="size-4 shrink-0 text-[color:var(--muted)]" /><span className="min-w-0 flex-1"><span className="block text-xs font-extrabold">Une question pendant le parcours ?</span><span className="mt-1 block text-[9px] text-[color:var(--muted)]">Le Guide connaît chaque étape.</span></span><ArrowRight className="size-4 text-[color:var(--muted)] transition group-hover:translate-x-1" /></button>
      </div>
    </aside>
  );
}
