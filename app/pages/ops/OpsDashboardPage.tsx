...
import { ArrowRight, CalendarCheck2, FileWarning, Inbox, MessageSquareText, UserCheck } from "lucide-react";
import { Card } from "@/components/ui/Card";
import { ButtonLink } from "@/components/ui/Button";
import { operationsRepository } from "@/services/supabase/operations";
import { usePageMeta } from "@/hooks/usePageMeta";
import { leadStatusLabel } from "@/config/lead-statuses";

const qualifiedStatuses = new Set(["qualified", "appointment_booked", "proposal_sent", "won"]);
...
export default function OpsDashboardPage() {
  ...
  const metrics = [
    { label: "Nouveaux leads · 7 j", value: leads.filter((lead) => Date.parse(lead.createdAt) >= since).length, icon: Inbox, href: "/ops/leads" },
  ...
  const funnel = [
    ["Reçus", leads.length],
    ["Contactés", leads.filter((lead) => !["new", "to_contact", "out_of_scope", "micro_only"].includes(lead.status)).length],
    ["Qualifiés", leads.filter((lead) => qualifiedStatuses.has(lead.status)).length],
    ["Gagnés", leads.filter((lead) => lead.status === "won").length],
  ] as const;
